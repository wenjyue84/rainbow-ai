...
import { cartAddItem, cartFormatSummary, type CartItem } from '../assistant/cart-store.js';
...
import { startSetMeal, formatChoicePrompt } from '../assistant/set-meal-store.js';
...
export function registerSearchHandlers(handlers: HandlerMap, sessionId: string): void {
  ...
  handlers.set('cart_search_item', async (args: any) => {
    ...
    if (menuItems.length === 0) {
      ...
      return {
        content: [{ type: 'text', text: `Added ${qty}x ${query} to your cart.\n\nCurrent cart:\n${cartFormatSummary(items)}` }]
      };
    }
    ...
    // Single unavailable match — suggest alternatives (US-854)
    if (matches.length === 1 && unavailableMatch) {
      ...
      return {
        content: [{ type: 'text', text: `Sorry, ${unavailableMatch.name} is currently out of stock.${alternativesText}` }]
      };
    }
    ...
    if (effectiveMatches.length === 1) {
      ...
      // US-865: Set meal with choices
      if (match.choices && match.choices.length > 0) {
        const pending = startSetMeal(sessionId, { name: match.name, code: match.code, price: match.price, qty, notes, choices: match.choices });
        transitionOrderStage(sessionId, 'ORDERING');
      ...
      // US-877: Allergen check
      if (match.code) {
        const allergenEntry = getAllergenEntry(match.code);
        if (allergenEntry && (allergenEntry.allergens.length > 0 || allergenEntry.dietary_flags.length > 0)) {
      ...
      transitionOrderStage(sessionId, 'ORDERING');
      return {
        content: [{ type: 'text', text: `Added ${qty}x ${match.name}${priceStr} to your cart.\n\nCurrent cart:\n${cartFormatSummary(items)}` }]
      };
    }
  ...
  handlers.set('cart_pick_item', async (args: any) => {
    ...
    if (chosen.choices && chosen.choices.length > 0) {
      const pending = startSetMeal(sessionId, { name: chosen.name, code: chosen.code, price: chosen.price, qty, notes, choices: chosen.choices });
      transitionOrderStage(sessionId, 'ORDERING');
    ...
    transitionOrderStage(sessionId, 'ORDERING');
    return {
      content: [{ type: 'text', text: `Added ${qty}x ${chosen.name}${priceStr} to your cart.\n\nCurrent cart:\n${cartFormatSummary(items)}` }]
    };
  });
}
