...
export class StateManager<T> {
  ...
  private onExpire?: (key: string, state: T & StatefulEntry<T>) => void;

  /**
   * @param ttlMs Time-to-live in milliseconds (default: 1 hour)
   * @param cleanupIntervalMs Cleanup interval in milliseconds (default: 5 minutes)
   * @param options Optional callbacks (e.g. onExpire for persistence cleanup)
   */
  constructor(
    private ttlMs: number = 3_600_000,
    private cleanupIntervalMs: number = 300_000,
    options?: { onExpire?: (key: string, state: T & StatefulEntry<T>) => void }
  ) {
  ...
  /**
   * Set state directly (used for hydrating from DB on startup).
   * Sets `lastActiveAt` if not already present.
   */
  set(key: string, state: T): void {
    const entry = {
      ...state,
      lastActiveAt: (state as any).lastActiveAt || Date.now()
    } as T & StatefulEntry<T>;
  ...
  private cleanupExpired(): void {
    ...
    for (const key of toDelete) {
      if (this.onExpire) {
        ...
        if (state) this.onExpire(key, state);
      }
      this.store.delete(key);
    }
