...
// Makan-profile specific workflows (no pelangi profileId)
const VALID_WORKFLOWS_MAKAN = JSON.stringify({
  workflows: [
    {
      id: 'booking_flow',
      name: 'Order Flow',
...
describe('ProfileIntegrityChecker — test_detects_cross_profile_keywords', () => {
  ...
  beforeEach(() => {
    ...
    writeFixture(dir, 'workflows.json', VALID_WORKFLOWS_MAKAN);
  });
  ...
  it('test_detects_cross_profile_keywords: detects Pelangi capsule terms in data-makan routing.json', () => {
    ...
    // 'capsule_conflict' contains 'capsule' — either term triggers contamination detection
    const allMessages = contamIssues.map(i => i.message).join(' ');
    expect(allMessages).toMatch(/capsule/);
  });
