/**
 * US-485: Profile Isolation Validation Test Suite — Makan Profile
 *
 * Tests that the Makan cafe profile maintains strict data isolation:
 * - intent-keywords.json contains zero hostel/room/guest-related keywords
 * - Knowledge base .md files contain zero hostel-specific keywords
...
describe('Profile Isolation Validation — Makan Profile (US-485)', () => {
  let report: ProfileContaminationReport;

  describe('Setup', () => {
    it('should validate makan profile without throwing', () => {
      expect(() => {
        report = validateProfileIsolation('makan');
      }).not.toThrow();
    });
  ...
  describe('Intent Keywords Isolation', () => {
    it('should not contain hostel-specific keywords in intent-keywords.json', () => {
      ...
      if (intentViolations.length > 0) {
        ...
        expect(violatedCategories.has('hostel_specific')).toBe(true);
      }
    ...
    it('should have zero violations for hostel-specific keywords', () => {
      const hostelViolations = report.violations.filter(
        v => v.keyword_category === 'hostel_specific'
      );
      expect(hostelViolations).toEqual([]);
    });
  ...
  describe('Knowledge Base Isolation', () => {
    it('should not contain hostel-specific keywords in .md files', () => {
      const kbViolations = report.violations.filter(
        v => v.file.endsWith('.md')
      );

      // Test that makan KB has no hostel-specific keywords
  ...
  describe('Edge Cases', () => {
    it('should handle missing intent-keywords.json gracefully', () => {
      ...
      expect(() => {
        const result = validateProfileIsolation('makan');
